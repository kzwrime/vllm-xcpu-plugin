import torch
import torch_xcpu  # 保留导入，但不再直接调用._C
...
def _register_silu_and_mul_op():
    ...
    try:
        torch.library.define(
            "torch_xcpu::silu_and_mul", 
            "(Tensor(a!) out, Tensor input) -> Tensor(a!)"
        )
        
        @torch.library.impl("torch_xcpu::silu_and_mul", "CPU")
        def _silu_and_mul_impl(out: torch.Tensor, input: torch.Tensor):
            try:
                d = input.shape[-1] // 2
                a = input[..., :d]
                b = input[..., d:]
                ...
                if input.dtype == torch.float32 and _is_torch_xcpu_op_available("silu_and_mul_fp32"):
                    torch.ops.torch_xcpu.silu_and_mul_fp32(out, a, b)
                elif input.dtype == torch.bfloat16 and _is_torch_xcpu_op_available("silu_and_mul_bf16"):
        ...
        @torch.library.impl("torch_xcpu::silu_and_mul", "CPU")
        def _silu_and_mul_fallback(out: torch.Tensor, input: torch.Tensor):
            d = input.shape[-1] // 2
            a = input[..., :d]
            b = input[..., d:]
            out.copy_(torch.nn.functional.silu(a) * b)
...
class XcpuSiluAndMul(SiluAndMul):
    ...
    def forward_cpu(self, input: torch.Tensor) -> torch.Tensor:
        ...
        # 2. 保留核心逻辑，无任何日志输出
        if input.size(-1) % 2 != 0:
            raise ValueError(f"SiluAndMul input last dim must be even, got {input.size(-1)}")
        
        d_out = input.shape[-1] // 2
        out = torch.empty(*input.shape[:-1], d_out, dtype=input.dtype, device=input.device)
